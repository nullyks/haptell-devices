...
const HTTP_HOST = process.env.HAPTELL_03_WEB_HOST || "127.0.0.1";
const HTTP_PORT = Number(process.env.HAPTELL_03_WEB_PORT || 8081);
...
function sendUdpCommand(ipAddress, port, command) {
  return new Promise((resolve, reject) => {
    ...
    socket.send(payload, port, ipAddress, (error) => {
      socket.close();
      if (error) {
        reject(error);
        return;
...
async function handleSend(request, response) {
  try {
    ...
    await sendUdpCommand(ipAddress, port, command);
    sendJson(response, 200, { ok: true, sent: command, ipAddress, port });
    console.log(`Sent to ${ipAddress}:${port} -> ${command}`);
  } catch (error) {
...
server.listen(HTTP_PORT, HTTP_HOST, () => {
  console.log(`Haptell 03 frequency web sender running at http://${HTTP_HOST}:${HTTP_PORT}`);
