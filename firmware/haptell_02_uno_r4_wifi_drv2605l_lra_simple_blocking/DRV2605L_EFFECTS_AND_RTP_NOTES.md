...
- how the web Shape Designer command maps to realtime playback
...
## Shape Designer Command

The simple blocking sketch also supports the same command generated by the
Node.js Shape Designer:

```text
haptell-02 shape duration=1600 points=0:0,100:180,700:180,1200:60,1600:0
```

The sketch parses the `points` list, configures unsigned/unidirectional RTP, and
linearly interpolates between each `time:intensity` pair until the requested
duration is finished.

This example remains blocking. While a `shape` is playing, the sketch does not
read new UDP packets, so `stop` cannot interrupt it immediately. Use the main
non-blocking `haptell-02` firmware when immediate interruption is needed.
