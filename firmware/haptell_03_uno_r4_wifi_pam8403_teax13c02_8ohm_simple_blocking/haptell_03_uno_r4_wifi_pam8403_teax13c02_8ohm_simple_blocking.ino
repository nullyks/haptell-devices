...
void setup() {
  ...
  Serial.println("Haptell haptell-03 PAM8403/TEAX13C02-8ohm simple blocking starting");
...
void playPattern(String command) {
  ...
  playLoadedPattern((unsigned int)durationMs);
}
...
void playLoadedPattern(unsigned int durationMs) {
  if (patternPointCount == 0) {
    stopPlayback();
    return;
  }

  Serial.print("Playing blocking frequency pattern for ");
